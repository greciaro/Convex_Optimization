...
##### RESULTS ARRAYS SENSITIVITY ##############
TANKDIAMETERArray = zeros(50,50)
NUMBEROFTANKSArray = zeros(50,50)
COSTOFTHERMALENERGYSTORAGEArray = zeros(50,50)

TANKRADIUS = zeros(1,50)
NUMBEROFTANKS = zeros(1,50)

# Create an outer loop sweeping across starting point of the shipment variable

for ii=1:50
  TANKRADIUS[ii] = 0.125+((2.5-0.125)/50)*ii

  for jj=1:50
  NUMBEROFTANKS[jj] = 1+((25-1)/50)*jj
  ...
  #m = Model(solver=IpoptSolver(max_iter=1000))
  m = Model(solver = AmplNLSolver(joinpath(PATH_TO_SOLVERS,"knitro"), ["outlev=2"]))

  ###### DECISION VARIABLES #########################
  # Number of tanks
  @variable(m, NTanks >=1,start=NUMBEROFTANKS[jj]) #[#]
  # Internal redius of tanks
  @variable(m, rTankIn >=0,start=TANKRADIUS[ii])#[m]
  # Thickness of Steel
  ...
  #### PRINTING RESULTS ####
  COSTOFTHERMALENERGYSTORAGEArray[ii,jj] = getobjectivevalue(m)
  TANKDIAMETERArray[ii,jj] = getvalue(rTankIn)*2
  NUMBEROFTANKSArray[ii,jj] = getvalue(NTanks)

  end
end

# Create simple bar and line charts for your decision variables
surface(TANKDIAMETERArray,title="Tank Diameter Sensitivity Analysis", ylabel="Tank Diameter[m]", xlabel="Initial values of tank radius (0.12-2.5[m]) and Number of tanks (1-25)",label="")
surface(NUMBEROFTANKSArray,title="Number of Tanks Sensitivity Analysis", ylabel="Number of Tanks", xlabel="Initial values of tank radius (0.12-2.5[m]) and Number of tanks (1-25)",label="")
heatmap(TANKDIAMETERArray,title="Tank Diameter Sensitivity Analysis", ylabel="Intial value of Number of tanks (1-25)", xlabel="Initial values of tank radius (0.12-2.5[m])",label="")
heatmap(NUMBEROFTANKSArray,title="Number of Tanks Sensitivity Analysis", ylabel="Intial value of Number of tanks (1-25)", xlabel="Initial values of tank radius (0.12-2.5[m]) and Number of tanks (1-25)",label="")

CSV.write("Cost_G.csv",DataFrame(COSTOFTHERMALENERGYSTORAGEArray))
CSV.write("Diam_G.csv",DataFrame(TANKDIAMETERArray))
CSV.write("Tanks_G.csv",DataFrame(NUMBEROFTANKSArray))
